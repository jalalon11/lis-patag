                ...
                <td v-for="day in daysOfWeek" :key="day.id" class="position-relative p-0" style="min-height: 400px;">
                  ...
                  <div class="p-2">
                    <button 
                      ...
                      class="btn btn-outline-primary btn-sm w-100  hover-opacity-100 transition-all"
                      style="border-style: dashed; border-width: 2px; padding: 10px;"
                    >
                      <i class="fas fa-plus mb-1"></i>
                      <br>
                      <small class="fw-semibold">Add Class Schedule</small>
                    </button>
                  </div>
                </td>
              </tr>
  ...
  methods: {
    ...
    saveSchedule() {
      ...
      const url = this.editingSchedule 
        ? `/admin/schedules/${this.editingSchedule.id}`
        : '/admin/schedules'
      
      const method = this.editingSchedule ? 'put' : 'post'
      
      this.$inertia[method](url, this.scheduleForm, {
        onSuccess: () => {
          this.saving = false
          ...
          showSuccessToast(
            this.editingSchedule ? 'Schedule Updated!' : 'Schedule Added!',
